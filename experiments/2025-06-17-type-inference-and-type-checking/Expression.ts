...
export class ExpressionPrinter {
  static print(expr: Expression, indent: number = 0): string {
    switch (expr.kind) {
      ...
      case "Application":
        return `${this.print(expr.function, indent)}(${
          this.print(expr.argument, indent)
        })`;

      case "Let":
        return `let ${expr.variable} = ${this.print(expr.value, indent)} in ${
          this.print(expr.body, indent)
        }`;
      ...
      case "Match": {
        const cases = Array.from(expr.cases.entries())
          .map(([tag, matchCase]) =>
            `${tag}(${matchCase.parameter}) => ${
              this.print(matchCase.body, indent + 1)
            }`
          )
          .join(", ");
      ...
      case "Handle": {
        const handlers = Array.from(expr.handlers.entries())
          .map(([op, handler]) => {
            ...
            return `${op}(${params}, ${handler.resumeParameter}) => ${
              this.print(handler.body, indent + 1)
            }`;
          })
          .join(", ");
        return `handle ${
          this.print(expr.expression, indent)
        } { ${handlers}, return => ${this.print(expr.returnCase, indent)} }`;
      }
