...
export class WitGenerator {
  ...
  private typeKindToString(kind: WitTypeKind): string {
    switch (kind.kind) {
      ...
      case "record":
        return `record { ${
          kind.fields.map((f) => `${f.name}: ${this.typeRefToString(f.type)}`)
            .join(", ")
        } }`;
      case "variant":
        return `variant { ${
          kind.cases.map((c) =>
            `${c.name}${c.type ? `(${this.typeRefToString(c.type)})` : ""}`
          ).join(", ")
        } }`;
      case "enum":
      ...
      case "union":
        return `union { ${
          kind.types.map((t) => this.typeRefToString(t)).join(", ")
        } }`;
      case "option":
  ...
  private typeRefToString(
    ref: string | { name: string; package?: string },
  ): string {
  ...
  private writeFunction(func: WitFunction): void {
    const params = func.params.map((p) =>
      `${p.name}: ${this.typeRefToString(p.type)}`
    ).join(", ");
