...
import { ParserContext, synchronize, token, seq, SyncPredicate } from "./Parser.ts";
...
describe("synchronize combinator integration", () => {
  ...
  it("should work with statement boundary synchronization", () => {
    ...
    while (context.peek()?.kind === "Whitespace" || context.peek()?.kind === "Newline") {
      context.consume();
    }
    ...
    const recoveringParser = synchronize(letDeclarationParser, syncToNextStatement);
    ...
    expect(currentToken.kind === "Newline" || currentToken.kind === "let").toBe(true);
  });
  ...
  it("should work with custom sync predicates using context", () => {
    ...
    const recoveringParser = synchronize(dataDeclarationParser, syncToNextDeclaration);
