...
export class TypeFactory {
  ...
  static handler(
    effect: EffectType,
    returnType: Type,
    operations: Record<string, HandlerOperation>,
  ): HandlerType {
    return {
      kind: "HandlerType",
      effect,
...
export class CommonTypes {
  ...
  static either<L extends Type, R extends Type>(
    left: L,
    right: R,
  ): ApplicationType {
...
export class TypePrinter {
  static print(type: Type, depth: number = 0): string {
    switch (type.kind) {
      ...
      case "FunctionType": {
        const params = type.parameters.length === 0 ? "()" : type.parameters
          .map((p, i) => `arg${i}: ${this.print(p, depth + 1)}`)
          .join(", ");

        const effects = type.effects.length > 0
          ? ` /* effects: ${
            type.effects.map((e) => this.print(e, depth + 1)).join(", ")
          } */`
          : "";
      ...
      case "RecordType": {
        ...
        return depth === 0
          ? `{\n${fields}${tail}\n}`
          : `{ ${
            Array.from(type.row.fields.entries()).map(([name, t]) =>
              `${name}: ${this.print(t, depth + 1)}`
            ).join("; ")
          } }`;
      }
      ...
      case "ApplicationType": {
        ...
        const args = type.arguments.map((arg) => this.print(arg, depth + 1))
          .join(", ");
        ...
        if (constructor === "Either") {
          const [left, right] = type.arguments.map((arg) =>
            this.print(arg, depth + 1)
          );
      ...
      case "ForallType": {
        ...
        const typeParams = type.variables.map((v) => {
          const constraint = v.variance
            ? ` /* ${this.printVariance(v.variance)} */`
            : "";
        ...
        if (type.body.kind === "FunctionType") {
          ...
          const effects = funcBody.effects.length > 0
            ? ` /* effects: ${
              funcBody.effects.map((e) => this.print(e, depth + 1)).join(", ")
            } */`
            : "";

          return `<${typeParams}>(${params}) => ${
            this.print(funcBody.returnType, depth + 1)
          }${effects}`;
        }
            ...
            const params = op.parameters.length === 0
              ...
              : `(${
                op.parameters.map((p, i) =>
                  `arg${i}: ${this.print(p, depth + 1)}`
                ).join(", ")
              })`;
            return `  ${name}${params}: ${
              this.print(op.returnType, depth + 1)
            }`;
          })
          .join(";\n");
        ...
        const ops = Array.from(type.operations.entries())
          .map(([name, op]) => {
            const params = op.parameters.length === 0
              ? "()"
              : `(${
                op.parameters.map((p: Type, i: number) =>
                  `arg${i}: ${this.print(p, depth + 1)}`
                ).join(", ")
              })`;
            const continuation = `(resume: (value: ${
              this.print(op.continuationType, depth + 1)
            }) => void)`;
            return `  ${name}${params}${
              params === "()" ? continuation : `, ${continuation}`
            }: ${this.print(op.resultType, depth + 1)}`;
          })
          .join(";\n");
  ...
  static describe(type: Type): string {
    switch (type.kind) {
      ...
      case "FunctionType": {
        ...
        const effectDesc = effectCount > 0
          ? ` with ${effectCount} effect(s)`
          : "";
...
export function createExamples() {
  const examples = {
    ...
    functions: {
      ...
      map: CommonTypes.map(),
      addNumbers: TypeFactory.func(
        [TypeFactory.number(), TypeFactory.number()],
        TypeFactory.number(),
      ),
    },
