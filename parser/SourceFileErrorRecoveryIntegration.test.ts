...
import { DiagnosticCollection, DiagnosticSeverity, DiagnosticCode } from "../diagnostics/mod.ts";
...
function createParserContext(source: string, fileName: string = "test.ts"): ParserContext {
...
function parseStatementsWithRecovery(source: string): {
    ...
    if (result.type === "success") {
      ...
      while (!context.isAtEnd() && tokensSkipped < 20) { // Prevent infinite loops
        ...
        if (token.kind === "Newline" || 
            (token.kind === "Symbol" && (token.symbol === "Semicolon" || token.symbol === "CloseBrace"))) {
          break;
        }
      }
    }
  ...
  return { statements, errors: totalErrors + context.diagnostics.getAll().length, context };
}
...
describe("Source File Error Recovery Integration Tests", () => {
  ...
  describe("Multiple Syntax Errors - Single Pass Detection", () => {
    it("should find errors using manual statement parsing with recovery", () => {
      ...
      const { statements, errors, context } = parseStatementsWithRecovery(source);
      ...
      const letDecls = statements.filter(stmt => stmt instanceof AST.LetDeclaration);
  ...
  describe("Well-formed Code After Errors", () => {
    it("should parse correct code after syntax errors using manual recovery", () => {
      ...
      // Should have let declarations
      const letDecls = statements.filter(stmt => stmt instanceof AST.LetDeclaration);
      const funcDecls = statements.filter(stmt => stmt instanceof AST.FunctionDeclaration);
  ...
  describe("Error Recovery Non-Interference", () => {
    it("should not interfere with parsing of completely correct code", () => {
      ...
      if (result.type === "success") {
        ...
        const functions = statements.filter(stmt => stmt instanceof AST.FunctionDeclaration);
        const letDecls = statements.filter(stmt => stmt instanceof AST.LetDeclaration);
