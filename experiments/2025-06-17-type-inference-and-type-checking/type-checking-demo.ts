...
import { TypeFactory, CommonTypes, TypePrinter, createExamples } from './TypeHelpers.ts'
...
export function runTypeCheckerDemo() {
  ...
  console.log('Extended person <: Person?', checker.isSubtype(extendedPerson, person))
  console.log('Person <: Extended person?', checker.isSubtype(person, extendedPerson))
  console.log('')
  ...
  const appliedList = checker.applyTypeConstructor(List, [TypeFactory.boolean()])
  ...
  console.log('List<Dog> <: List<Animal>? (covariance)', checker.isSubtype(listOfDogs, listOfAnimals))
  console.log('')
  ...
  console.log('Identity instantiated for numbers:', TypePrinter.print(numberIdentity))
  
  const stringIdentity = checker.instantiate(identity, [TypeFactory.string()])
  console.log('Identity instantiated for strings:', TypePrinter.print(stringIdentity))
  ...
  const numberOnlyIdentity = TypeFactory.func([TypeFactory.number()], TypeFactory.number())
  ...
  console.log('Number identity <: Polymorphic identity?', checker.isSubtype(numberOnlyIdentity, numberIdentity))
  console.log('')
  ...
  const funcNumberToString = TypeFactory.func([TypeFactory.number()], TypeFactory.string())
  const funcAnyToString = TypeFactory.func([TypeFactory.typeVar('any')], TypeFactory.string())
  ...
  const funcAnimalToString = TypeFactory.func([animalType], TypeFactory.string())
  ...
  console.log('(Animal) -> string <: (Dog) -> string? (contravariant params)', checker.isSubtype(funcAnimalToString, funcDogToString))
  console.log('(Dog) -> string <: (Animal) -> string? (contravariant params)', checker.isSubtype(funcDogToString, funcAnimalToString))
  console.log('(Animal) -> Dog <: (Animal) -> Animal? (covariant return)', checker.isSubtype(funcAnimalToDog, funcAnimalToAnimal))
  console.log('(Animal) -> Animal <: (Animal) -> Dog? (covariant return)', checker.isSubtype(funcAnimalToAnimal, funcAnimalToDog))
  console.log('')
  ...
  // Extend the row
  const extendedRow = checker.extendRow(baseRow, new Map([
    ['email', TypeFactory.string()],
    ['phone', TypeFactory.string()]
  ]))
  
  console.log('Base row:', TypePrinter.print({ kind: 'RecordType', row: baseRow }))
  console.log('Extended row:', TypePrinter.print({ kind: 'RecordType', row: extendedRow }))
  
  // Restrict the row
  const restrictedRow = checker.restrictRow(extendedRow, ['phone'])
  console.log('Restricted row (removed phone):', TypePrinter.print({ kind: 'RecordType', row: restrictedRow }))
  
  // Demonstrate row subtyping
  const baseRecord = TypeFactory.record({ name: TypeFactory.string(), age: TypeFactory.number() })
  ...
  console.log('Base record <: Extended record?', checker.isSubtype(baseRecord, extendedRecord))
  console.log('Extended record <: Base record? (width subtyping)', checker.isSubtype(extendedRecord, baseRecord))
  console.log('')
  ...
  console.log('Handler is compatible with effect?', checker.checkHandler(stateHandler, stateEffect))
  
  // Demonstrate effect subtyping (more effects <: fewer effects)
  const pureFunction = TypeFactory.func([TypeFactory.number()], TypeFactory.number(), [])
  const ioFunction = TypeFactory.func([TypeFactory.number()], TypeFactory.number(), [ioEffect])
  const statefulIoFunction = TypeFactory.func([TypeFactory.number()], TypeFactory.number(), [stateEffect, ioEffect])
  ...
  console.log('Pure <: IO function? (effect subtyping)', checker.isSubtype(pureFunction, ioFunction))
  console.log('IO <: Stateful+IO function? (effect subtyping)', checker.isSubtype(ioFunction, statefulIoFunction))
  console.log('Stateful+IO <: IO function? (effect subtyping)', checker.isSubtype(statefulIoFunction, ioFunction))
  console.log('')
  ...
  const stringToNumberMap = checker.instantiate(mapType, [TypeFactory.string(), TypeFactory.number()])
  console.log('Map instantiated for string->number:', TypePrinter.print(stringToNumberMap))
  
  // Create more specific map functions to show subtyping
  const animalToStringMap = checker.instantiate(mapType, [animalType, TypeFactory.string()])
  const dogToStringMap = checker.instantiate(mapType, [dogType, TypeFactory.string()])
  ...
  console.log('Animal->string map <: Dog->string map? (function contravariance)', checker.isSubtype(animalToStringMap, dogToStringMap))
  console.log('Dog->string map <: Animal->string map? (function contravariance)', checker.isSubtype(dogToStringMap, animalToStringMap))
  console.log('')
  ...
  console.log('Network error result <: Error result?', checker.isSubtype(networkErrorResult, errorResult))
  console.log('')

  // Demo 9: TypeScript-friendly syntax showcase
  console.log('=== 9. TypeScript Syntax Showcase ===')
  
  console.log('🎯 Notice how all types are printed in familiar TypeScript syntax:')
  console.log('')
  
  console.log('• Functions use arrow syntax:', TypePrinter.print(TypeFactory.func([TypeFactory.string()], TypeFactory.number())))
  console.log('• Records use object syntax:', TypePrinter.print(TypeFactory.record({ name: TypeFactory.string(), age: TypeFactory.number() })))
  console.log('• Generics use <T> syntax:', TypePrinter.print(CommonTypes.list(TypeFactory.string())))
  console.log('• Options become nullable:', TypePrinter.print(CommonTypes.Some(TypeFactory.string())))
  console.log('• Arrays use Array<T>:', TypePrinter.print(CommonTypes.list(TypeFactory.number())))
  console.log('• Effects become interfaces:', TypePrinter.print(CommonTypes.IOEffect))
  console.log('')
