...
import { ParserContext, ParseError } from "./Parser.ts";
...
function createContext(
  ...
  return new ParserContext("test.ts", tokens, new DiagnosticCollection(), config);
}
...
describe("RecoveryPerformanceManager", () => {
  ...
  describe("Recovery attempt limits", () => {
    ...
    it("should prevent recovery when limit is reached", () => {
      const config = { ...DEFAULT_RECOVERY_PERFORMANCE_CONFIG, maxRecoveryAttempts: 3 };
  ...
  describe("Recovery point caching", () => {
    ...
    it("should return null when caching is disabled", () => {
      const config = { ...DEFAULT_RECOVERY_PERFORMANCE_CONFIG, enableRecoveryPointCaching: false };
    ...
    it.skip("should evict old cache entries when cache is full", () => {
      const config = { ...DEFAULT_RECOVERY_PERFORMANCE_CONFIG, recoveryPointCacheSize: 2 };
      ...
      const context1 = new ParserContext("file1.ts", tokens1, new DiagnosticCollection());
      const context2 = new ParserContext("file2.ts", tokens2, new DiagnosticCollection());
      const context3 = new ParserContext("file3.ts", tokens3, new DiagnosticCollection());
      
      const baseTime = Date.now();
      const point1 = { position: 0, diagnosticCount: 0, timestamp: baseTime - 2000 };
      const point2 = { position: 0, diagnosticCount: 0, timestamp: baseTime - 1000 };
  ...
  describe("Bounded token skipping", () => {
    ...
    it("should respect global token skip limit", () => {
      const config = { ...DEFAULT_RECOVERY_PERFORMANCE_CONFIG, maxTokensToSkip: 2 };
      const limitedManager = new RecoveryPerformanceManager(config);
      
      const tokens = Array.from({ length: 10 }, (_, i) => createToken("Identifier", `test${i}`));
      const context = createContext(tokens);
...
describe("BoundedRecoveryHistory", () => {
  ...
  describe("Event management", () => {
    ...
    it("should handle circular buffer correctly", () => {
      const events = [
        { strategy: "test1", position: 0, tokensSkipped: 1, success: true, message: "msg1" },
        { strategy: "test2", position: 1, tokensSkipped: 2, success: true, message: "msg2" },
        { strategy: "test3", position: 2, tokensSkipped: 3, success: true, message: "msg3" },
        { strategy: "test4", position: 3, tokensSkipped: 4, success: true, message: "msg4" }, // Should overwrite first
      ];
...
describe("BoundedContextStack", () => {
  ...
  describe("Stack operations", () => {
    ...
    it("should reject pushes when at maximum depth", () => {
      const contexts = [
        { name: "test1", expectedElements: [], recoveryStrategies: [], metadata: {} },
        { name: "test2", expectedElements: [], recoveryStrategies: [], metadata: {} },
        { name: "test3", expectedElements: [], recoveryStrategies: [], metadata: {} },
        { name: "test4", expectedElements: [], recoveryStrategies: [], metadata: {} }, // Should be rejected
      ];
...
describe("ParserContext Performance Integration", () => {
  ...
  describe("Recovery attempt limits", () => {
    it("should respect recovery attempt limits", () => {
      const config = { ...DEFAULT_RECOVERY_PERFORMANCE_CONFIG, maxRecoveryAttempts: 2 };
      const tokens = [createToken("Identifier", "test")];
      ...
      const error = ParseError.error(DiagnosticCode.UNEXPECTED_TOKEN, "Test", tokens[0].span);
  ...
  describe("Context stack limits", () => {
    it("should respect context stack depth limits", () => {
      const config = { ...DEFAULT_RECOVERY_PERFORMANCE_CONFIG, maxContextStackDepth: 2 };
      const tokens = [createToken("Identifier", "test")];
      const context = createContext(tokens, config);
      
      const context1 = { name: "test1", expectedElements: [], recoveryStrategies: [], metadata: {} };
      const context2 = { name: "test2", expectedElements: [], recoveryStrategies: [], metadata: {} };
      const context3 = { name: "test3", expectedElements: [], recoveryStrategies: [], metadata: {} };
      ...
      const warnings = diagnostics.filter(d => d.code === DiagnosticCode.PARSER_LIMIT_EXCEEDED);
  ...
  describe("Recovery history management", () => {
    it("should manage recovery history size", () => {
      const config = { ...DEFAULT_RECOVERY_PERFORMANCE_CONFIG, maxRecoveryHistorySize: 3 };
      const tokens = [createToken("Identifier", "test")];
      const context = createContext(tokens, config);
